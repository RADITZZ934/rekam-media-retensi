<template>
  <div class="min-h-screen bg-gray-50 p-8">
    ...
    <div class="max-w-2xl mx-auto space-y-6">
      <!-- Section: Automation & Developer Tools -->
        ...
        <div class="p-8">
          <div class="mb-8">
            ...
            <div class="flex bg-gray-100 p-1 rounded-xl mb-4 max-w-[200px]">
              <button 
                type="button"
                @click="settings.retention_update_unit = 'hours'; showMockOcr = false"
                :class="[
              ...
              <button 
                type="button"
                @click="settings.retention_update_unit = 'minutes'; showMockOcr = false"
                :class="[
                  'flex-1 py-1.5 text-xs font-semibold rounded-lg transition-all',
                  settings.retention_update_unit === 'minutes' 
                    ? 'bg-white text-blue-600 shadow-sm' 
            ...
            <!-- Mock OCR Toggle -->
            <div v-if="settings.retention_update_unit === 'minutes' && showMockOcr" class="mt-6 pt-6 border-t border-gray-100 flex items-center justify-between">
              <span class="text-sm font-semibold text-gray-700">Mock OCR</span>
              <button 
                type="button" 
                @click="settings.mock_ai_interceptor = !settings.mock_ai_interceptor"
                :class="settings.mock_ai_interceptor ? 'bg-blue-600' : 'bg-gray-200'"
            ...
            <div class="mt-8 p-4 bg-blue-50/50 rounded-xl border border-blue-100 flex gap-4">
              ...
              <div class="text-[11px] text-blue-800 leading-relaxed">
                ...
                <p>Sistem akan menjalankan tugas sinkronisasi setiap <span @click="toggleMockOcr" class="font-bold underline cursor-pointer hover:text-blue-600 select-none">{{ settings.retention_update_interval }} {{ settings.retention_update_unit === 'minutes' ? 'menit' : 'jam' }}</span> sekali. Ini mencakup pembaruan status retensi pasien dan otomatisasi data pemusnahan.</p>
              </div>
            </div>
          </div>
...
const showMockOcr = ref(false)

const toggleMockOcr = () => {
  if (settings.value.retention_update_unit === 'minutes') {
    showMockOcr.value = !showMockOcr.value
  }
}
